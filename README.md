...
3. *(First run only)* Pull the Llama 3 weights into the local container:
```bash
...
If you wish to experiment with this limit, you can easily change it by modifying the `iterations >= 8` threshold inside the `should_continue` function in `engine.py`.
